...
# Ref: https://github.com/flairNLP/flair/blob/master/resources/docs/TUTORIAL_5_DOCUMENT_EMBEDDINGS.md
import numpy as np 
import torch 
...
stop_words.append('shape')
stop_words.append('shapes')
...
class ConvE(torch.nn.Module):
    def __init__(self, args, num_entities, num_relations):
        ...
        #self.ent_weights_matrix = torch.nn.Embedding(num_entities, args.emb_dim, padding_idx=0)
        ...
        self.fc = torch.nn.Linear(43648,args.emb_dim)
    ...
    def init_entemb_glove(self):
        ...
        for i in range(self.ne):
            ...
            not_found_list = []
            found_list_vec = []
            #print(self.identmap[i])
            glove_ent = self.identmap[i]
            try:
    ...
    def init_entemb_bert(self):
        self.entitymap, self.identmap = self.ent2id()
        # init embedding
        embedding = TransformerDocumentEmbeddings('bert-base-cased')
                
        for i in range(self.ne):
            sentence = Sentence(self.identmap[i])
            embedding.embed(sentence)
            #print(sentence.get_embedding())
            # embed the sentence
            self.ent_weights_matrix[i] = sentence.get_embedding().detach().cpu().numpy()
    ...
    def init(self):
        ...
        #self.init_entemb_glove()
        self.init_entemb_bert()
    ...
    def forward(self, e1, rel, print_pred=False):
        ...
        e1_embedded= self.emb_e(e1).view(-1, 1, 12, 64)
        rel_embedded = self.emb_rel(rel).view(-1, 1, 12, 64)
...
def train(args):
    ...
    for ep in tqdm(range(args.max_epochs)):
        ...
        for counter in range(num_train_steps):
            ...
            for t_id in t:
                e2_multi[0][t_id] = 1.0
            e2_multi = ((1.0-args.label_smoothing_epsilon)*e2_multi) + (1.0/e2_multi.shape[1]) # Label smoothing
...
def get_kg_ent_substring(ent, model, entitynames_dict, contains_shape=True):
    #print('In get kg ent substring match')
    ...
    if contains_shape:
        ...
        ent_tokens = tz.tokenize(ent_tokens)

        for i, ent in enumerate(ent_emb):
            key_ent = entitynames_dict[i]
            key_ent_tokens = tz.tokenize(key_ent)
            # Find number of substring matches between the tokenized entity and the user provided utterance
            entscore_dict[i] = len(set(key_ent_tokens) & set(ent_tokens))
        
        target_ent_idx = max(entscore_dict, key=entscore_dict.get)
    kg_ent = entitynames_dict[target_ent_idx]
    #print('Chose entity:{}'.format(ent))
    return kg_ent  

def get_kg_ent_bert(ent, model, entitynames_dict, contains_shape=True):
    #embedding = TransformerDocumentEmbeddings('roberta-base')
    embedding = TransformerDocumentEmbeddings('bert-base-cased')
    #from flair.embeddings import SentenceTransformerDocumentEmbeddings

    # init embedding
    #embedding = SentenceTransformerDocumentEmbeddings('bert-base-cased')
    ...
    if contains_shape:
        ...
        # ent_tokens = ' '.join(ent_tokens)
        query_sentence = Sentence(ent)
        print('Query sentence:{}'.format(query_sentence))
        embedding.embed(query_sentence)

        np_query_vec = query_sentence.get_embedding().detach().cpu().numpy()        
        
        for i, ent_ in enumerate(ent_emb):
            np_ent = ent_.numpy()
    ...
    kg_ent = entitynames_dict[target_ent_idx]
    print('Chose entity:{}'.format(kg_ent))
    return kg_ent

def get_kg_ent_glove(ent, model, entitynames_dict, contains_shape=True):
    global stop_words
    # Pick a random entity id in case there are errors, we don't return null
    target_ent_idx = 100
    if contains_shape:
        ...
        for i, ent in enumerate(ent_emb):
            np_ent = ent.numpy()
            cos_sim = cosine(np_query_vec, np_ent)
            entscore_dict[i] = 1 - cos_sim
...
def infer_glove(args, ent, rel, contains_shape=True):
    global stop_words
    global word2vector
    global word_vocab
    data = WikiData(args)
    ...
    ent = get_kg_ent_glove(ent, model, entitynames_dict, contains_shape=True)
    ...
    for j, id in enumerate(pred[0].cpu().detach().numpy()): 
        ...
        kg_mem_dict[pred_entity] = top_k_scores[j]
        #top_k_scores.append(ent_score)
    #print("--------------------------------------------------------------------------")
    #print(mem_dict)
    return ent, top_k_scores, top_k_entities, kg_mem_dict

def infer_substringmatch(args, ent, rel, contains_shape=True):
    global stop_words
    global word2vector
    global word_vocab
    data = WikiData(args)
    ...
    ent = get_kg_ent_substring(ent, model, entitynames_dict, contains_shape=True)
...
def infer_bert(args, ent, rel, contains_shape=True):
    data = WikiData(args)
    num_entities =  len(data.entity_ids)
    num_relations =  len(data.rel_ids)
    ...
    ent = get_kg_ent_bert(ent, model, entitynames_dict, contains_shape)
    ...
    with open(args.scoringfilepath, 'r') as f:
        ...
        for line in lines:
            ...
            for index, utt in enumerate(utterances):
                ...
                #kg_ent, top_k_scores, top_k_entities, kg_mem_dict = infer_substringmatch(args, entities[0], new_rel, contains_shape)
                #kg_ent, top_k_scores, top_k_entities, kg_mem_dict = infer_glove(args, entities[0], new_rel, contains_shape)
                kg_ent, top_k_scores, top_k_entities, kg_mem_dict = infer_bert(args, entities[0], new_rel, contains_shape)

                print ("==========>", kg_ent, new_rel)
                print ("==========>", kg_mem_dict)

                candidates[index] = kg_mem_dict
                if index == 0:
                    for k, v in kg_mem_dict.items():
                            mem_dict[k] = 0
                
                elif index > 0:
                    for ii in range(index):
                        prev_mem_dict = candidates[index-1]
                        for k, v in kg_mem_dict.items():
                            if k in prev_mem_dict:
                                if k in mem_dict:
                                    mem_dict[k] += prev_mem_dict[k]
                                    #mem_dict[k] += v
                                else:
                                    mem_dict[k] = prev_mem_dict[k]
                                    #mem_dict[k] = v
            ...
            print('Top 1, Top 3, Top 5 ents ====> {},{},{} '.format(top_1_ent, top_3_ent, top_5_ent))
            print("--------------------------------------------------------------------------")
...
def score_shapes(args):
    with open(args.scoringfilepath, 'r') as f:
        lines = f.readlines()
        #print("--------------------------------------------------------------------------")
        for line in lines:
            shape_desc = line.strip()
            rel = relation_extraction(shape_desc)
            new_rel = rel + '_reverse'
            entities = entity_extraction(shape_desc)
            if not entities[0]:
                entities = [shape_desc]
                contains_shape = True
            if 'Shape' in rel:
            ...
            # kg_ent, top_k_scores, top_k_entities = infer_glove(args, entities[0], new_rel, contains_shape)
            # top_k_scores, top_k_entities = infer_bert(args, entities[0], new_rel, contains_shape)
            # kg_ent, top_k_scores, top_k_entities = infer_substringmatch(args, entities[0], new_rel, contains_shape)
            print ("{};{}".format(shape_desc, kg_ent))
...
def main(args):
    ...
    # Model inference
    # start_time = time.time()
    # infer_glove(args, 'Rectangle with a chunk cut out', 'HasShape_reverse')
    # end_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelpath', dest='modelpath', default='models/conve_bert.pt', help='Path to save the trained model to')
    parser.add_argument('--scoringfilepath', dest='scoringfilepath', default='data/descriptions_for_select_target_sim_3_des_10_times_with_postentshapes.txt', help='Path to file containing the utterances to be used for scoring')
    ...
    parser.add_argument('--emb_dim', dest='emb_dim', default=768, help='Entity and relation embedding dimensions')
    ...
    parser.add_argument('--epochs', dest='max_epochs', default=5, help='Max epochs')
    
    parser.add_argument('--top_k', dest='top_k', default=20, help='Top K vals to consider from the predictions')
